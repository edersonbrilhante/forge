...
resource "signalfx_detector" "runner_log_delivery_health" {
  ...
  description = "Monitors the shared runner-log Firehose source reader, Splunk delivery path, and dead-letter queue. Idle delivery streams do not alert."
  max_delay   = 120
...
source_incoming = data('IncomingRecords', filter=(${local.control_plane_filter}) and filter('namespace', 'AWS/Kinesis') and filter('StreamName', 'splunk-s3-runner-logs-stream-*') and filter('stat', 'sum'), rollup='sum', extrapolation='zero').sum(over='5m').sum(by=['aws_region'])
...
detect(when(source_throttles > 0, '5m'), off=when(source_throttles == 0, '10m')).publish('Runner-log Firehose source throttled')
EOF
  ...
  rule {
    description   = "Runner-log Firehose source reader remains more than five minutes behind Kinesis for 10 minutes"
    severity      = "Warning"
    detect_label  = "Runner-log Firehose source lag warning"
    notifications = var.detector_notifications
  }

  rule {
    description   = "Runner-log Firehose source reader remains more than 15 minutes behind, or stops reading while input continues, for 10 minutes"
    severity      = "Critical"
    detect_label  = "Runner-log Firehose source lag critical"
    notifications = var.detector_notifications
  }

  rule {
    description   = "Runner-log Firehose source reader is throttled for 5 minutes"
    severity      = "Major"
    detect_label  = "Runner-log Firehose source throttled"
    notifications = var.detector_notifications
  }
}
