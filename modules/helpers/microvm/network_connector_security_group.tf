...
resource "aws_security_group" "connector" {
  #checkov:skip=CKV2_AWS_5:The security group is consumed inside the CloudFormation-managed Lambda Network Connector rather than by a Terraform-native ENI resource.
  for_each = var.network_connectors

  name        = "forge-microvm-${each.value.name}-${var.aws_region}"
